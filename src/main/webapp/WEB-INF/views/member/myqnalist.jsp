			...
			<div class="pager">
				...
				<ul class="-list- center">
						...
						<c:if test="${startNum>1 }">
							<a class="btn btn-prev" href="/mysiteB/qna?a=listmyqna&p=${startNum-1 }}">◀</a>
						</c:if>
						<c:forEach var="i" begin="0" end="4">
						<c:if test="${(startNum+i) <= lastNum}">	
							<a class="${(param.p==(startNum+i))?'selected':'' }"></a><a href="/mysiteB/qna?a=listmyqna&p=${startNum+i}">${startNum+i}</a>						
						</c:if>
						</c:forEach>
						
						<c:if test="${startNum+4<lastNum }">
							<a href="/mysiteB/qna?a=listmyqna&p=${startNum+5}"class="btn btn-next">▶</a>
						</c:if>
				</ul>
			</div>
